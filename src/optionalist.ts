...
type OptionInformation =
  | OptionInformationString
  | OptionInformationNumber
  | OptionInformationBoolean;
...
type OptionType<type extends OptionInformation['type']> = type extends 'boolean'
  ? true
  : type extends 'number'
  ? number
  : string;
        ...
        switch (info.type) {
          ...
          case 'number':
            if (typeof info.default !== 'number') {
              throw new Error(
                `The default value of the ${optArg} parameter must be a number.: ${info.default}`
              );
            }
            break;
          case undefined:
          case 'string':
            if (typeof info.default !== 'string') {
              throw new Error(
                `The default value of the ${optArg} parameter must be a string.: ${info.default}`
              );
            }
            break;
          default:
    ...
    while (true) {
      ...
      switch (info.type) {
        ...
        case 'number': {
          ...
          if (value === undefined) {
            throw new Error(
              `${arg} needs a number parameter${
                info.example ? ' as the ' + info.example : ''
              }`
            );
          }
