...
(function (this: any) {
  ...
  this.Symbol.isSymbol = (symbol: unknown): symbol is symbol => String(symbol) in symbols;

  const registered: {[key: string]: symbol} = {};

  this.Symbol.for = (key: string) => (registered[key] ??= Symbol(key));
  this.Symbol.keyFor = (sym: symbol) => Object.entries(registered).find(([, symbol]) => symbol === sym)?.[0];
